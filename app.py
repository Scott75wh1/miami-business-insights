...
# Attempt to import OpenAI, handle missing module
try:
    import openai
...
def generate_pdf_report(neighs, search_term, df_demo, df_trends, df_competitors):
    ...
    pdf.cell(0,8, f'Query: {search_term}', ln=1)
    pdf.ln(5)
    # Demographics
    if not df_demo.empty:
        pdf.set_font('Arial','B',14)
        pdf.cell(0,8,'Dati Demografici',ln=1)
        pdf.set_font('Arial', size=12)
    ...
    if not df_trends.empty:
        pdf.set_font('Arial','B',14)
        pdf.cell(0,8,'Google Trends (ultimo valore)',ln=1)
        pdf.set_font('Arial', size=12)
        pdf.cell(0,6, str(df_trends['trend_volume'].iloc[-1]), ln=1)
        pdf.ln(5)
    # Competitors
    if not df_competitors.empty:
        pdf.set_font('Arial','B',14)
        pdf.cell(0,8,'Competitor',ln=1)
        pdf.set_font('Arial', size=12)
...
selected = st.sidebar.multiselect('Quartieri:', list(NEIGHBORHOODS.keys()), default=['Downtown'])
...
if st.sidebar.button('Analizza'):
    ...
    if enable_demo:
        ...
        for n in selected:
            z = NEIGHBORHOODS[n]
            df = fetch_demographics_by_zip(z, CENSUS_API_KEY)
            if not df.empty:
                df['neighborhood'] = n
                dfs.append(df)
...
for _, r in df_comp.iterrows():
    ...
    # Export PDF
    if st.sidebar.button('Esporta PDF'):
        pdf_bytes = generate_pdf_report(selected, search_term, df_demo, df_trends, df_comp)
