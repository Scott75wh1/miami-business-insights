...
# Inizializza lo stato di generazione
...
if st.session_state.generate:
    ...
    # Demografici
    if enable_demo:
        with tabs[0]: st.subheader(f"Dati Demografici per {area}")
        try:
            ...
            if not df_demo.empty:
                ...
                chart.columns = [area]
                st.bar_chart(chart)
            else: st.warning("Nessun dato demografico disponibile.")
        except Exception as e: st.error(f"Errore demografici: {e}")
    # Trends
    if enable_trends:
        with tabs[1]: st.subheader(f"Google Trends: {search_term}");
        ...
        else: st.warning("Nessun dato Google Trends disponibile.")
    # Competitor
    if enable_google or enable_yelp:
        with tabs[2]: st.subheader(f"Analisi Competitor: {search_term}")
        if enable_google:
            ...
            else: st.warning("Nessun risultato Google Reviews.")
        if enable_yelp:
            st.markdown("**Yelp**")
            ...
            else: st.warning("Nessun risultato Yelp.")
    # Mappa
    with tabs[3]: st.subheader("Mappa Interattiva dei Competitor")
